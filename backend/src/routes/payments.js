...
paymentsRouter.use(requireAuth, requireActiveBusiness);

paymentsRouter.get("/summary", (req, res) => {
  res.json(db.getPaymentsSummary(req.businessId));
});

paymentsRouter.get("/", (req, res) => {
  res.json({ payments: db.listPayments(req.businessId, req.query) });
});

paymentsRouter.post("/booking-payments", (req, res) => {
